...
class DEPRECATEDContentAccessAndStorageApiSimulation extends Simulation {
...
// Setup http protocol configuration
    ...
    val doDeletePATHFeeder = csv(userDataDirectory + File.separator + "doDelete-pathParams.csv").random
    ...
    val scndoCreate = scenario("doCreateSimulation")
        ...
        .exec(http("doCreate")
        .httpRequest("PUT","/api/${type}/${name}/${path}")
)
    ...
    val scndoDelete = scenario("doDeleteSimulation")
        ...
        .exec(http("doDelete")
        .httpRequest("DELETE","/api/${type}/${name}/${path}")
)
    ...
    val scndoGet_0 = scenario("doGet_0Simulation")
        .feed(doGetPATHFeeder)
        .exec(http("doGet_0")
        .httpRequest("GET","/api/${type}/${name}/${path}")
)
    ...
    val scndoHead = scenario("doHeadSimulation")
        ...
        .exec(http("doHead")
        .httpRequest("HEAD","/api/${type}/${name}/${path}")
